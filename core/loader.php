...
class BPGE extends BP_Group_Extension {
    ...
    function __construct(){
        ...
        if( isset($bp->groups->current_group->extras['display_page']) && $bp->groups->current_group->extras['display_page'] == 'public' ){
                ...
                <ul>';
                        foreach($pages as $page){
                            echo '<li '. (($bp->action_variables[0] == $page->post_name) ? 'class="current"' : '') .'>
                                <a href="'.bp_get_group_permalink( $bp->groups->current_group ) . $this->page_slug .'/'. $page->post_name.'">'.stripslashes($page->post_title).'</a>
                            </li>';
                        }
    ...
    function gpages_screen_content(){
        ...
        if(!empty($page)){
            ...
            echo '<div class="gpage">';
                ...
                if (bpge_user_can('group_extras_admin')){
                    echo '<div class="edit_link"><a target="_blank" title="'.__('Edit link for group admins only', 'bpge').'" href="'.bp_get_group_permalink( $bp->groups->current_group ).'admin/extras/pages-manage/?edit='.$page->ID.'">'.__('[Edit this page]', 'bpge') .'</a></div>';
                }
            echo '</div>';
        }
    ...
    function edit_screen_general($bp123){
        global $bp;
        // check user access to group extras management pages
        if(!bpge_user_can('group_extras_admin'))
            return;
        ...
        $group_nav = bpge_nav_order();//$bp->bp_options_nav[$bp->groups->current_group->slug];
    ...
    function edit_screen_pages_manage($bp){
        ...
        echo '<p>';
            ...
            echo '<input type="text" value="'.esc_attr($page->post_title).'" name="extra-page-title">';
        echo '</p>';
